...
def create_queues(channel):
    ...
    create_specific_queue(channel, 'purchase_ticket_log')
    create_specific_queue(channel, 'purchase_ticket_error')

# function to create Activity_Log queue  
def create_specific_queue(channel, queue_name):
    print(f'amqp_setup:create_{queue_name}_queue')
    channel.queue_declare(queue=queue_name, durable=True)
    channel.queue_bind(exchange=exchangename, queue=queue_name, routing_key=f'{queue_name}.#')
