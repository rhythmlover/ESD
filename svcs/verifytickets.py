...
def orchestratewithsingpass(): 
    ...
    ticket_status_response = requests.get(f"http://localhost:5011/get-ticket-status?ticket_id={ticket_id}")
    ...
    age_verification_response = requests.get(f"http://localhost:5010/verify-age?UEN={UEN_id}&UNIFIN={UNIFIN_id}")
    ...
    update_response = requests.post(f"http://localhost:5009/update-verified?ticket_id={ticket_id}&UEN={UEN_id}")
