...
def verify_ticket():
    ...
    try:
        ...
        if UEN_id and UNIFIN_id:
            ...
            result = orchestratewithoutsingpass(ticket_id, UEN_id, qr_code)
            print('\n------------------------')
            print('\nresult: ', result)
            return result
...
def orchestratewithsingpass(ticket_id, UEN_id, UNIFIN_id, qr_code):
    ...
    ticket_status_response = requests.get(
        f"http://host.docker.internal:5009/get-ticket-status/{ticket_id}/{qr_code}")
    ...
    update_response = requests.post("http://host.docker.internal:5001/update-verified", json=update_data)
    
    if update_response.status_code == 201:
        return jsonify({'message': 'Ticket verification and update completed successfully'}), 201
    else:
...
def orchestratewithoutsingpass(ticket_id, UEN_id, qr_code):
    if not ticket_id:
    ...
    try:
        ...
        if 'ticket_redeemed' in ticket_info and ticket_info['ticket_redeemed']:
            ...
            update_data = {
                'ticket_id': ticket_id,
                'UEN_id': UEN_id  # We're not using UEN_id in this case
            }
            ...
            if update_response.status_code == 201:
                return jsonify({'message': 'Ticket appended to user successfully.'}), 201
            else:
                error_message = f"Failed to update verified status. Status code: {update_response.status_code}, Response: {update_response.json()}"
